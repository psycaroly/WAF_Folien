...
date: "2025/06/16 (updated: `r Sys.Date()`)"
output:
...
### Einheit 9: Auswertung von Studien - Teil 2: Datenvisualisierung und Rückmeldung
##### 17.06.2025 | Prof. Dr. Caroline Zygar-Hoffmann
...
- Vorhergesagte Werte kommen auf die y-Achse, nicht auf die x-Achse
...
https://github.com/MI2-Education/2021Z-DataVisualizationTechniques/blob/master/projects/project1/tomaszewski_bialonczyk_ziemla/tomaszewski_bialonczyk_ziemla.pdf <br> https://x.com/Ricarda_Lang/status/1932486128378007613</span></div>
...
```{r eval = TRUE, echo = F, out.width = "75%"}
knitr::include_graphics("bilder/legende1.png")
```

```{r eval = TRUE, echo = F, out.width = "75%"}
knitr::include_graphics("bilder/legende2.png")
```
]
...
* Ist diese Abbildung notwendig? 
* Ist die Abbildung einfach, klar und frei von störenden Details? 
* Sind alle Elemente der Abbildung klar gekennzeichnet? 
* Werden alle Abkürzungen und Symbole in einer Legende oder Beschriftung erklärt? 
* Sind die Abbildungen von den Termen, Abkürzungen und dem Stil her konsistent? 
* Haben die Abbildungen keinen Rahmen (APA-Vorgabe)? 
* Sind die Größenordnungen, Skalen und Richtungen der Achsen klar gekennzeichnet? 
* Sind parallele Abbildungen gleich relevant und in derselben Skala? 
* Ist die Auflösung der Graphiken hoch genug? 
* Sind alle Graphiken sequentiell durchnummeriert? 
* Wird auf alle Graphiken im Bericht verwiesen?
* Falls urheberrechtlich geschützte Abbildungen verwendet werden, wurde der Urheber vollständig angegeben? Haben Sie eine schriftliche Druck-Erlaubnis für diese Graphiken eingeholt? 
* Sind die Graphiken in dem geforderten Format (z.B. vom Journal)? 
...
**In der Regel sollte die Normstichprobe in der Rückmeldung detailliert beschrieben werden.** Dies ist bei Variante 2 nur eingeschränkt möglich, da zumindest die eigenen Themen aus Anonymitätsgründen nur wenige Informationen über die Stichprobe erhoben haben.
...
* Rmarkdown Vorlage zur Rückmeldung  "[template_rueckmeldung.Rmd](https://studynet.hs-fresenius.de/goto_STUDYNETHSF_file_46051_download.html)" von studynet herunterladen
* ggf. packages installieren, die im R-Skript geladen werden
* Ganz oben bei "title" den Titel Ihrer Studie einfügen
* Angabe des Messinstruments und Beschreibung des Maßes anpassen
* R-Code anpassen: 
  - Zeile 8,9, 12 und 42: Wie heißt Ihr vorverarbeiteter Datensatz, den Sie für die Analysen einlesen werden (im Template ist es `final_data`)?
...
* In Zeile 13 ein Pseudonym einfügen, das auch wirklich in Ihren Daten existiert
...
* Entweder manuell in Zeile 13 immer wieder: 1) das Pseudonym austauschen 2) neu knitten 3) erhaltene Datei umbenennen (sonst wird sie überschrieben -> umbenennen in das Pseudonym, z.B. `ZvGEUeAkly.html`)
* Oder das R-Skript auf studynet "[template_rueckmeldung_automate.R](https://studynet.hs-fresenius.de/goto_STUDYNETHSF_file_46052_download.html)" ausprobieren (da bei großen Stichproben das manuelle Vorgehen ewig dauern würde)
  ...
  - im Rmarkdown-Skript `template_rueckmeldung.Rmd` muss dann Zeile 13 auskommentiert werden (mit einem `#`), dafür Zeile 14 nicht mehr (das `#` löschen)
...
* Im studynet Ordner "Rückmeldungen" im Unterordner mit dem Titel der eigenen Studie die HTML Dateien hochladen
