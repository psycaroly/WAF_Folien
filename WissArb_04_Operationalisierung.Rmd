...
date: "2024/05/03 (updated: `r Sys.Date()`)"
output:
...
##### 08.05.2024 | Dr. Caroline Zygar-Hoffmann
...
* ein Beispielbericht aus dem letzten Semester ist nun [auf studynet zu finden](https://studynet.hs-fresenius.de/ilias.php?baseClass=ilrepositorygui&ref_id=28354)

* von der Bibliothek erreichte mich folgender Hinweis, welcher ggf. für Ihre weitere Literaturrecherche relevant ist:

" *Zugriff auf lizenzierte eRessourcen erfolgt künftig ausschließlich über die Bibliothekssuche des Opera Browsers auf dem Terminalserver bzw. RDS/Remote Desktop. Andere Browser sind nicht für den Zugriff auf die eRessourcen freigeschaltet.
Die Nutzung des Proxy wird in den nächsten Tagen eingestellt. Bitte installieren Sie deshalb rechtzeitig den Remote Desktop/RDS. Die Anleitung befindet sich in der Wissensdatenbank unter folgendem Link*:
https://service.crf-education.com/external/knowledge-base/article/49 "
...
oder "Video X: z.B. https://www.youtube.com/watch?v=JGwWNGJdvx8"
]
...
Zunächst werden Ihre Daten auf zugangsbeschränkten Servern des Leibniz-Rechenzentrums (LRZ) gespeichert. Die Daten werden dann auf Rechnern der am Projekt beteiligten Personen (siehe Punkt 7) verarbeitet.
...
* Erweitern Sie das [Präregistrierungstemplate auf studynet](https://studynet.hs-fresenius.de/goto_STUDYNETHSF_file_27541_download.html) (AP3, AP4, M12, M13)
...
* Nutzen Sie das formr-Template auf studynet für [eigene Themen](https://studynet.hs-fresenius.de/goto_STUDYNETHSF_file_28654_download.html) oder [das Interventionsthema](https://studynet.hs-fresenius.de/goto_STUDYNETHSF_file_28655_download.html).

* Bevor Sie anfangen im Template rumzuwerken: 
  - Benennen Sie die Excel-Datei in einen eindeutigen(!) Namen für Ihre Studie um.
  - Laden Sie Ihre umbenannte Excel-Datei als Survey hoch und testen das Survey mal, so dass Sie sehen, wie sich die Excel-Datei in eine Umfrage übersetzt.
...
* Bitte alle Anpassungen im Template (d.h. zusätzlich eingefügten Items/Videos etc.) farbig markieren.

* Zu Test-Zwecken können Sie auch bestimmte Seiten/Items aus dem Template erstmal rauslöschen (am Ende werden Sie aber Teil der finalen Studie sein).

* Falls Sie einen zusätzlichen Song untersuchen wollen: Es wäre am besten, wenn es den Song auf YouTube gibt (wenn nicht, wenden Sie sich bitte an mich). Wenn Sie das Lied auf YouTube öffnen und auf "Teilen" klicken, können Sie hier "Einbetten" auswählen. Sie müssen dann den Link der hinter src= steht kopieren und anstelle des Videos im Template einfügen. Ich bin noch dabei herauszufinden, wie man das Video ausblendet. Bitte teilen Sie mir mit, falls es für Ihre Forschungsfrage wichtig ist, dass das Video mit abgespielt wird. Aktuell sind nur Lyrics + Musik geplant.

* Variable für die Rückmeldung wird erst an einem späteren Punkt entschieden, das müssen Sie erstmal nicht beachten.
...
$\rightarrow$ Sobald alles in formr fertig getestet ist und Sie mir den Run exportiert haben, werde ich die Studie auf einer anderen formr-Instanz hochladen (zur Wahrung des Datenschutzes und damit niemand versehentlich Daten löscht). Nach Abschluss der Datenerhebung stelle ich jeder Kleingruppe ihren Datensatz bereit.
...
**Folgende Gruppen mit Interventionsthema gehen nun in Raum P3 107 mit Nensy Le**
- Beziehungsdauer und Musik (Isabelle Wörl, Michael Djatlow, Jana Bookmeyer, Tizian Neumaier, Katharina Knüfer)
- Cohens Crew (Natalie Berger, Amelie Debusmann, Jorina van Loo, Julia Ziegler, Jakob Höfling)
- Sexforscher (Amelie Fritz, Annalena Marek, Max Zausinger)

**Folgende Gruppen mit eigenem Thema gehen nun in Raum P3 110 mit Caroline Zygar-Hoffmann**
- Die menstruierenden Forscherinnen :) (Melanie Antelmann, Elisa Schönacher, Lena Jegielka, Nina Schönecker, Lilith Tinkhauser)
- Die Grashüpfer (Emma Pfister, Marie Luckinger, Isabella Riedl, Ayse Bilir, Karoline Grünwaldt)
- Falscher Optimismus (Marlene Babl, Jil Wagner, Franziska Stöhr, Simon Panetta, Sara Baddakhel)
- Extraversion (Emily Badstübner, Klaudia Kamrau, Rosalie Gehlen, Lucy Ramm, Lilli Herrmann)
- Die Bindungsforscher*innen (Alix Vischer, Lukas Ohlig, Corinna Sichart, Mascha Konzen, Rebecca Wekesser)
