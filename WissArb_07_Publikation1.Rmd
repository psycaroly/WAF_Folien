...
date: "2025/05/19 (updated: `r Sys.Date()`)"
output:
...
##### 20.05.2025 | Prof. Dr. Caroline Zygar-Hoffmann
...
* Zur Abgabe der Prüfungsleistung: Upload auf studynet, kein Ausdruck erforderlich; konkrete Hinweise folgen zum Semesterende

* Wenn Sie mit mir etwas individuell abgesprochen haben, ich aber nicht Ihr Ansprechpartner außerhalb der Vorlesung bin, dann würde ich empfehlen dies in Ihren GoogleDocs-Open-Lab-Books explizit zu notieren/erwähnen

* Das gilt ins Besondere für Absprachen die die Bewertung betreffen $\rightarrow$ Ihr Ansprechpartner wird Ihre Arbeit bewerten!

* **Wie viele Personen von der Interventionsgruppen rekrutiert werden sollen**: Die Poweranalysen der Interventionsthemen ergaben eine nötige Gesamtstichprobe von 171-956. Bei insgesamt 30 Personen im Interventionsthema ergibt sich, dass jeder aus einer Interventionsgruppe zwischen 6 (171/30) und 32 (956/30) Personen rekrutieren müsste. Das ist also unser „formales“ Ziel. 
<br><br>
Es passiert nichts, wenn wir dieses Ziel nicht erreichen – aber Sie können ja mal mit diesen Zahlen im Kopf schauen wie weit Sie mit dem Rekrutieren kommen. Bitte nennen Sie Ihren rekrutierten Personen Ihren Gruppennamen, damit wir zuordnen können wie viele Personen von jeder Gruppe bereits rekrutiert wurden (das sollen die Versuchspersonen im Fragebogen angeben).
...
* Es sind insgesamt 11+2(Nachholer) = 13 Studien. Schön wäre, wenn jeder an mindestens(!) 6 Studien teilnimmt (jede Studie sollte in wenigen Minuten erledigt sein)

* Falls Sie sich aus zeitlichen Gründen für eine Studie aus mehreren entscheiden müssen: Vorrang haben die Studien mit eigenem Thema, da für diese nur innerhalb der Kohorte rekrutiert wird.

---
class: top, left

###  Hinweis aus Ihrem Prüfungsamt
...
Wie soll es am Ende aussehen? Siehe hier:
...
.pull-right[
```{r eval = TRUE, echo = F, fig.cap="S. 17 in Sedlmeier, P. & Renkewitz, F. (2018). Forschungsmethoden und Statistik für Psychologen und Sozialwissenschaftler (3.Auflage). Pearson.", out.width = "75%"}
...
.center[
```{r eval = TRUE, echo = F, out.width = "70%"}
knitr::include_graphics("bilder/bewertung_form2.png")
```
]
...
* Darstellung **relevanter Forschungsergebnisse** aus wissenschaftlich hochwertiger, möglichst aktueller Literatur zu konkreten Studienergebnissen zu den zentralen Variablen, Theorien, Konzepten (vereinzelt ist auch ältere Literatur in Ordnung, gerade auch bei Theorien) 
...
* **Stichprobenbeschreibung als Text**
  ...
  - Sie müssen die Stichprobe nur hinsichlich der Aspekte beschreiben, die Sie auch erhoben haben! Wenn Sie z.B. kein Alter erhoben haben, müssen Sie es logischerweise auch nicht berichten.
  - deskriptive Statistik zur Stichprobe findet man gelegentlich auch im Ergebnisteil - ist ok, solange sie irgendwo auftauchen
...
* **Versuchsdurchführung**
  ...
  - Das schließt das Studiendesign (z.B. Bedingungen), den Ablauf (inklusive z.B. der Reihenfolge von Fragebögen!), verwendete Materialien, Instruktionen etc. ein
...
---
class: top, left
<div class="footer"><span>Pusch, S., Schönbrodt, F. D., Zygar-Hoffmann, C., & Hagemeyer, B. (2021). Perception of communal motives in couples: accuracy, bias, and their associations with relationship length. <i>Journal of Research in Personality</i>, 91, 104060.</span></div>
...
* **Messinstrumente**
  ...
  - Angabe der Antwortskala (inkl. numerischer/verbaler Anker - *verbale Anker werden kursiv formatiert* - hier macht APA konkrete Vorgaben!)
  - Angabe der Reliabilität für die Skalen (z.B. McDonald's Omega, Cronbach's Alpha); aber: falls Sie eine Variable mit nur einem 1 Item gemessen haben (d.h. 1 Frage), dann können und müssen Sie keine Reliabilität angeben
...
knitr::include_graphics("bilder/beschreibung_material.png")
```
]
...
* **Beschreibung der statistischen Analyse**
  ...
  - Relevante Vorverarbeitung, wie Transformationen der Daten (Rekodierung, Aggregation, Zentrierung (z.B. Standardisierung),...), Umgang mit fehlenden Werten, Ausschluss von Daten
...
knitr::include_graphics("bilder/bewertung_methode.png")
```
]
...
Ecarnot, F., Seronde, M. F., Chopard, R., Schiele, F., & Meneveau, N. J. E. G. M. (2015). Writing a scientific article: A step-by-step guide for beginners. <i>European Geriatric Medicine, 6</i>(6), 573-579.

.center[
```{r eval = TRUE, echo = F, out.width = "80%"}
knitr::include_graphics("bilder/Ecarnot.png")
```
]
...
**Schritt 0: Datenerhebung vorantreiben/beginnen**
* Konkret werden die Interventionsgruppen von mir einen Link zum Testen zugeschickt bekommen - pro Gruppe möchte ich eine Rückmeldung, ob beim Testen alles geklappt hat!
* Sobald alle Präregistrierungen eingefroren sind und die Studie getestet wurde schicke ich den finalen Link für die Erhebung rum inkl. finale Excel-Dateien zum Nachvollziehen - dann kann das Rekrutieren losgehen!
* Gruppen mit eigenem Thema kriegen ihren Link sobald die Präregistrierung eingefroren ist und ich ihre formr-Dateien geprüft habe
...
* Bei 5 Leuten empfehle ich die folgende Aufteilung:
  ...
  - 2 Person Methodenteil (inkl. OSF-Projekt zur Bereitstellung von Material/Code/Daten, Erklärung folgt in Einheit 12)
