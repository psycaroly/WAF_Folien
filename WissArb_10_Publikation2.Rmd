...
date: "2025/06/24 (updated: `r Sys.Date()`)"
output:
...
### Einheit 10: Publikation von Studien mit Fokus auf Ergebnisteil und Diskussion
##### 24.06.2025 | Prof. Dr. Caroline Zygar-Hoffmann

---
class: top, left

### 3 Schmankerl aus dem Interventionsthema

.center[
Hier scheint versehentlich der Rekrutierungstext in den Link eingefügt:
```{r eval = TRUE, echo = F}
knitr::include_graphics("bilder/schmankerl1.png")
```
]
...
.center[
Beziehungsstatus "anderes":
```{r eval = TRUE, echo = F}
knitr::include_graphics("bilder/schmankerl2.png")
```
]
...
.center[
Der Hassle eines Teilnehmers: 

```{r eval = TRUE, echo = F, out.width = "50%"}
knitr::include_graphics("bilder/schmankerl3.png")
```
]
...
### Nacherhebung Pseudonym für das Interventionsthema

* Wie letzte Woche während der Vorlesung erklärt habe ich versäumt in die Studie die Frage einzubauen, ob man Teil dieser Vorlesung ist. 

* Eigentlich sollte wer Teilnehmer dieser Vorlesung ist danach gefragt werden, ob er/sie eine Rückmeldung erhalten möchte und ein Pseudonym angeben können. 

* In der Konsequenz konnte bisher niemand aus dieser Vorlesung sein Pseudonym angeben. 

* **Ich habe daher diese kurze 1-seitige Befragung angelegt, in der Sie ein Pseudonym nachtragen können, indem Sie nochmal Ihre demographischen Angaben machen.** Ich versuche dann anhand der demographischen Angaben das Pseudonym in den Datensatz der Interventionsstudie einzupflegen:

.center[
https://surveys.osc.lmu.de/pseudonym
]
...
.pull-right[
```{r eval = TRUE, echo = F}
knitr::include_graphics("bilder/abstract.png")
```
]
...
---
class: top, left
<div class="footer"><span>https://tinyurl.com/ybremelq <br> https://x.com/JohnHolbein1/status/1930618108714308049</span></div>
...
knitr::include_graphics("bilder/bewertung_abstract.png")
```
]
...
* Ergebnisse werden teils in den **Fließtext** integriert, teils durch **Tabellen und Grafiken** veranschaulicht.
...
* Erst sagen, dann belegen: Ergebnis zuerst ankündigen, dann mit Statistiken begründen

"As predicted, the results show that the groups differ significantly, with group $X$ having higher values on variable $A$ $(M = 1.2$, $SD = 0.2)$ than group $Y$ $(M = 0.2$, $SD = 0.2),$ $t(48) = 2.3$, $p = .026$."
...
.center[
```{r eval = TRUE, echo = F, out.width = "90%"}
knitr::include_graphics("bilder/bewertung_ergebnisse3.png")
```
]
...
#### Elemente des Ergebnisberichts

* Angabe der relevanten Kennzahlen: z.B. Mittelwerte bei Gruppenvergleichen, Korrelation bei Zusammenhangshypothesen (siehe nächste Folie)

* Angabe der statistischen Kennwerte mit Freiheitsgraden (= degrees of freedom, df) in Klammern, z.B. $t(df)= ...$; $\chi^2(df)=...$
  
* Angabe des $p$-Wertsauf 3 Nachkommastellen genau: z.B. $p = .021$
  * einzige Ausnahme: wenn p-Wert den Wert .001 unterschreitet, dann $p < .001$ (die Wahrscheinlichkeit kann nicht 0 betragen!) 

* Angabe von Effektstärken (z.B. Cohen's d)
  
**Formalia zur Angabe von Ergebnissen (Zahlen) und Variablen:**

* lateinische Variablenbezeichnungen (z.b. $b$, $t$, $p$) werden kursiv gesetzt, griechische jedoch nicht (z.B. $\alpha$)
* Zahlen nicht kursiv
...
* $t$-Test
  ...
  * entweder: 
  the groups differed, $\Delta_{M} = -51.46,$ $95\%$ $CI$ $[-104.87, 1.95]$, $t(49.09) = -1.94,$ $p = .059;$ $Cohen's$ $d = -0.55$, $95\%$ $CI$ $[-1.12, 0.02]$
 
  * oder: 
  group $X$ $(M = 1.2,$ $SD = 0.2)$ differed from group $Y$ $(M = 0.2$, $SD = 0.2)$, $t(48) = 2.3$, $p = .026;$ $Cohen's$ $d = -0.55$, $95\%$ $CI$ $[-1.12, 0.02]$ ...

* Korrelation: 

$r = .13$, $95\%$ $CI$ $[-0.13, 0.38]$, $p = .312$, $R^2 = .02$
]
...
* Regressionsparameter
  ...
  * standardisiert: $\beta = 0.64$, $95\%$ $CI$ $[-0.62, 1.89]$, $t(57) = 1.02$, $p = .312$
  * unstandardisiert: $b = 0.64$, $95\%$ $CI$ $[-0.62, 1.89]$, $t(57) = 1.02$, $p = .312$

* $F$-Tests

  * Regressionsmodell: $R^2 = .02$, $F(1, 57) = 1.04$, $p = .312$
  * ANOVA: $F(3, 54) = 683.77$, $p < .001$, $\eta^2=0.97$, $95\%$ $CI$ $[0.96, 1.00]$
  
* $\chi^2$-Test: $r_{G1}=41\%$, $r_{G2}=70\%$, $\chi^2_{79}<.001$
]
...
* Größe des in der Untersuchung ermittelten Effekts, auch als standardisierte Effektstärken, z.B. $r$, $Cohen's$ $d$, $R^2$, $\eta^2$, ...
...
* dem APA Style guide folgen: https://apastyle.apa.org/instructional-aids/numbers-statistics-guide.pdf
  ...
  - statistische Kennwerte (z. B. Mittelwerte, Standardabweichungen, Korrelationskoeffizienten etc.) mit zwei Nachkommastellen angegeben (Ausnahme: p-Werte)
...
* Keine Abbildung/Tabelle ist besser als eine überflüssige Abbildung/Tabelle

* **Jede Tabelle/Abbildung muss sequentiell nummeriert werden**
  - Tabelle 1, Tabelle 2, …
  - Abbildung 1, Abbildung 2, …
  
* **Integration in den Text unter Nennung der Abbildungs-/Tabellen-Nr.**: 
.center[„Wie aus Abbildung 5 ersichtlich wird...“; „Die Daten zur Fernsehnutzung in Tabelle 3 verdeutlichen...“]

* **Quellenangaben bei Abbildungen und Tabellen**: Wenn man nicht seine eigene Daten in einer Tabelle oder einer Abbildung veranschaulicht, muss auch hier gekennzeichnet werden, von wem die Daten/ Informationen stammen -> Quellenangabe!
...
* Abkürzungen und Fachbegriffe müssen für alle Tabellen/Abbildungen konsistent sein. Abkürzungen müssen erklärt werden, auch wenn sie im Text schon einmal erklärt wurden (siehe Alleinverständlichkeits-Kriterium)
...
* Der Titel einer Tabelle muss kurz aber deutlich sein. $\rightarrow$  Muss muss den Inhalt der Tabelle aus dem Titel erschließen können
...
---
class: top, left
<div class="footer"><span>Based on https://apastyle.apa.org/style-grammar-guidelines/tables-figures/sample-tables#regression</span></div>

### Was gehört in den Ergebnisteil?

#### Beispiel für eine Regressionstabelle

.center[
```{r eval = TRUE, echo = F, out.width = "60%"}
knitr::include_graphics("bilder/regtable.png")
```
]
...
* Ist der Tabellen-Titel kurz, prägnant, aufschlussreich und selbsterklärend? 
...
* Werden die Konfidenzintervalle für die wichtigsten Punktschätzungen angegeben? 
...
* Erster Absatz: **Kurze Zusammenfassung** (kürzer als der Abstract!)
  ...
  - Zusammenfassung/Knapper Bericht zu den Hauptergebnissen (nur in Worten, hier werden **keine Zahlen** mehr genannt)
  - keine detaillierte Wiederholung des Ergebnisteils!

---
class: top, left
<div class="footer"><span>Murphy, S. L., Merz, R., Reimann, L., & Fernández, A. (2024, November 13). Nonsignificance Misinterpreted as an Effect’s Absence in Psychology: Prevalence and Temporal Analyses. https://doi.org/10.31234/osf.io/hm2tu</span></div>

### Was gehört in den Diskussionsteil?

* Dann **Interpretation der Ergebnisse** (dieser Teil soll zusammen mit der Diskussion der Ergebnisse größeren Raum in der Diskussion einnehmen): 
  ...
  + Egal ob signifikant oder nicht, die gefundene Effektstärke sollte eingeordnet werden: **Richtung des Effekts konsistent mit der Hypothese?** Geschätzter Effekt größer/kleiner als erwartet? 
  
  + ggf. Übersetzung in "common language effect sizes" (siehe https://pubmed.ncbi.nlm.nih.gov/34881941/ und R package `compute.es`: https://cran.r-project.org/web/packages/compute.es/compute.es.pdf)
  
  + Achtung: Kausale Sprache nur bei experimentellen Designs angemessen und auch dort ist eine kritische Diskussion über Einschränkungen der Kausalität notwendig!
  
  + Beispiele für kausale Interpretationen: "X beeinflusst Y", "X verursacht Y", "X verhindert Y", "X wirkt auf Y", "X verbessert/verschlechtert Y", "X erklärt Y" (im Vgl. zu nicht kausaler Interpretation, "X hängt zusammen mit Y", "X ist assoziiert mit Y", "X korreliert mit Y", "X geht mit mehr/weniger Y einher", "X sagt Y vorher")
...
* **Diskussion der Ergebnisse** (dieser Teil soll zusammen mit der Interpretation der Ergebnisse größeren Raum in der Diskussion einnehmen): 
...
* **Zukünftige Forschung**: Weiterführende Überlegungen, erste Ideen für neue Fragestellungen und Hypothesen
...
---
class: top, left
### APA: Quantitative Design Reporting Standards (JARS-Quant)

Weiterführende Hilfe für das Schreiben des Papers (für alle Bereiche, nicht nur Ergebnisse und Diskussion): 
...
**1) An Studien teilnehmen:**

.pull-left[
https://surveys.osc.lmu.de/dieLetzten
...
**2) Feedback geben**
Haben Sie sich bei irgendeiner Teilnahme gedacht: Moment, diesen Aspekt am Studiendesign/der Frage fand ich seltsam? Ich glaube das führt in die Irre? Das wurde nicht berücksichtigt? Das könnte eine relevante Störvariable sein? Schreiben Sie der entsprechenden Gruppe (persönlich, z.B. per Email) Ihr Feedback (für das Interventionsthema gerne an mich, ich sammle die Nachrichten und leite sie an alle weiter).
