...
date: "2024/12/16 (updated: `r Sys.Date()`)"
output:
...
##### 16.12.2024 | Prof. Dr. Caroline Zygar-Hoffmann
...
https://surveys.osc.lmu.de/song (nicht wundern: Erhebung von NachholerInnen aus dem letzten Semester; es dürfen auch die Personen vom Interventionsthema teilnehmen)

https://surveys.osc.lmu.de/reflection (Interventionsthema; d.h. Personen vom Interventionsthema dürfen nicht teilnehmen)
...
```{r, eval = FALSE}
survey1$condition <- "Gemeinsamkeiten"
survey2$condition <- "Unterschiede"
```
...
* z.B. existiert die Variable "known" im Interventionsthema nur in den Bedingungen "Gemeinsamkeiten" und "Unterschiede", aber nicht in "Selbst". So fügen wir diese Variable (ohne Einträge, also mit `NA`s) zum dritten survey hinzu:
...
* Wenn Sie zum Beispiel in Ihrer H1 "Gemeinsamkeiten" mit "Unterschiede" vergleichen, könnte der Befehl lauten:

```{r, eval = FALSE}
data$condition_H1 <- NA
data$condition_H1 <- ifelse(data$condition_A == "Gemeinsamkeiten", 1, data$condition_H1)
data$condition_H1 <- ifelse(data$condition_A == "Unterschiede", 0, data$condition_H1)
```

* Der erste ifelse()-Befehl in Worten: "Wenn in einer Zeile des Datensatzes in der `condition_A`-Spalte "Gemeinsamkeiten" drinsteht, dann schreibe bitte in die Variable `condition_H1` den Wert 1 rein, und sonst behalte den Wert der vorher drin stand."
...
```{r, eval = FALSE}
data <- data[!is.na(data$condition_A) & data$condition_A == "Gemeinsamkeiten" |
               !is.na(data$condition_A) & data$condition_A == "Unterschiede",]
```

Der Befehl in Worten: Überschreibe das Objekt "data" mit den Zeilen vom Objekt "data" in denen die Spalte "condition" nicht leer ist und wo entweder "Gemeinsamkeiten" oder "Unterschiede" drinsteht, und behalte alle Spalten)
...
**Schritt 2: Preprocessing der Daten beginnen**
* Wenn Sie mit der Datenerhebung schon begonnen haben, können Sie jetzt schonmal einen Auszug der Daten von mir anfordern, um mit dem Preprocessing beginnen zu können
* Den Zeitpunkt dafür, wann Sie den finalen vollständigen Datensatz bekommen möchten, können Sie selbst entscheiden
* Teilen Sie mir daher mit, ob Sie erstmal nur einen Auszug wollen, oder schon den finalen Datensatz (bei letzterem schließe ich die Umfrage danach)
